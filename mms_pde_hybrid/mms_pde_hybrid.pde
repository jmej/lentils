...
String[] svgNames = {"yellow.svg", "blue.svg", "red.svg", "green.svg", "brown.svg", "orange.svg"};
PShape[] svgs = new PShape[6];
String[][] imageNames = {
...
void setup (){
  ...
  for (int c = 0; c < 6; c++){ //setup each color
    ...
    for (int i = 0; i < 3; i++){
      ...
      colors[c][i].resize(lentilSizes[i]*2, lentilSizes[i]*2);
    }
  }
  frameRate(24);
}
...
class BgCircle implements Comparable<BgCircle>{
  ...
  void display(){
    ...
    if (age < 180){ //~5 seconds
      ...
      //fill(c, alpha);
      shape (svgs[bgCircleColor], x-(currentSize*0.5), y-(currentSize*0.5), currentSize, currentSize);
